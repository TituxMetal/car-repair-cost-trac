...
import { formatDate, formatMileage, isOverdue, maintenanceCategoryLabels } from '@/lib/helpers'
...
export const MaintenanceTimeline = ({
  ...
  const upcomingEvents = events.filter(
    e => e.status === 'scheduled' && !isOverdue(e.scheduledDate, e.status)
  )
          ...
          <div className='flex items-start justify-between gap-3'>
            <div className='flex-1 space-y-2'>
              <div className='flex items-center gap-2 flex-wrap'>
                <WrenchIcon className='text-accent shrink' size={18} />
    ...
    <div className='space-y-6'>
      {overdueEvents.length > 0 && (
        <div className='space-y-3'>
          <div className='flex items-center gap-2'>
            <WarningIcon className='text-destructive' size={20} />
      ...
      {events.length === 0 && (
        <div className='text-center py-12'>
          <WrenchIcon className='text-muted-foreground mx-auto mb-4' size={48} />
